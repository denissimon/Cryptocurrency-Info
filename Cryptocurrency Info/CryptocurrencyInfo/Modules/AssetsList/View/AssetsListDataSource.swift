...
class AssetsListDataSource: NSObject {
    ...
    let presentDetails = Event<AssetDetailsViewController>()
...
extension AssetsListDataSource: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc = UIStoryboard(name: "Details", bundle: nil).instantiateViewController(withIdentifier: "Details") as! AssetDetailsViewController
        vc.navigationItem.title = data[indexPath.item].name
        vc.viewModel = AssetDetailsViewModel(networkService: NetworkService(), asset: data[indexPath.item])
        presentDetails.trigger(vc)
    }
